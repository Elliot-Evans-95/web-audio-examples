...
function midiNote(note) {
    const dataOctave = `[data-octave="${note.octave}"]`;
    const dataNote = `[data-note="${note.key}"]`;
    const elKey = document.querySelector(`${dataOctave}${dataNote}`);
...
const onEnabled = () => {
    ...
    launchkeyMiniMK3Input.addListener('noteon',
        e => {
            midiNote({key: e.note.name, octave: e.note.octave})
        }
    );

    launchkeyMiniMK3Input.addListener('noteoff',
        e => {
            midiNoteOff({key: e.note.name, octave: e.note.octave})
        }
    );
